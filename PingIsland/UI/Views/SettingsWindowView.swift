...
private struct SettingsPanelContentView: View {
    ...
    private var panelCornerRadius: CGFloat {
        presentation == .window ? 0 : (settings.visualThemeMode.isPixel ? 4 : 18)
    }
...
private struct SidebarItemView: View {
    ...
    var body: some View {
        HStack(spacing: 9) {
            ZStack(alignment: .topTrailing) {
                AgentIslandThemeSymbol(
                    ...
                    size: 14,
                    color: isSelected ? Color.accentColor : Color.white.opacity(0.58)
                )
        ...
        .frame(minHeight: 30)
        .background(
            RoundedRectangle(cornerRadius: theme.isPixel ? 3 : 7, style: .continuous)
                .fill(rowBackgroundColor)
        )
        .overlay {
            if theme.isPixel && isSelected {
                RoundedRectangle(cornerRadius: 3, style: .continuous)
                    .strokeBorder(Color(red: 0.30, green: 0.92, blue: 0.70).opacity(0.72), lineWidth: 2)
            }
        }
        .contentShape(RoundedRectangle(cornerRadius: theme.isPixel ? 3 : 7, style: .continuous))
        .onHover { isHovered = $0 }
    ...
    private var rowBackgroundColor: Color {
        if theme.isPixel {
            if isSelected {
                return Color(red: 0.30, green: 0.92, blue: 0.70).opacity(0.14)
            }
