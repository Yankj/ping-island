...
struct HookInstaller {
    ...
    static func bridgeHealthStatus() -> BridgeHealthStatus {
        ...
        guard FileManager.default.fileExists(atPath: socketPath) else {
            return BridgeHealthStatus(
                isHealthy: false,
                message: AppLocalization.string("Bridge 监听尚未启动，请保持 AgentIsland 正在运行后重试")
            )
        }
        ...
        guard runBridgeLauncherHealthCheck(launcherURL) else {
            return BridgeHealthStatus(
                isHealthy: false,
                message: AppLocalization.string("Bridge launcher 自检失败，请重启 AgentIsland 后重新安装 Hooks")
            )
        }
    ...
    private static func managedMarker(for profile: ManagedHookClientProfile) -> String {
        // Keep the upstream marker so AgentIsland can update an existing managed
        // entry instead of installing duplicate hooks beside Ping Island.
    ...
    static func managedPluginSource(for profile: ManagedHookClientProfile) -> String {
        ...
        return """
        // \(marker)
        // Generated by AgentIsland. Reinstall from Island settings if you need to refresh it.
        ...
        export const server = async ({ client, serverUrl }) => {
          ...
          return {
          name: "AgentIsland",
          description: "Forward OpenCode hook events to AgentIsland.",
          event: async ({ event }) => {
    ...
    static func managedPluginDirectoryFiles(
        ...
        name: ping_island
        version: 1.0.0
        description: Forward Hermes Agent plugin hooks to AgentIsland
        provides_hooks:
          - on_session_start
          - pre_llm_call
          - pre_tool_call
    ...
    private static func managedAntigravityPluginFiles(
        ...
        let manifest: [String: Any] = [
            ...
            "name": "ping-island",
            "description": "\(marker). Forward Antigravity CLI activity to AgentIsland."
        ]
            ...
            if (isDangerousBash(event)) {
              ...
              try {
                ...
                if (deniedByIsland(response)) {
                  return { block: true, reason: "Blocked by AgentIsland" };
                }
              } finally {
    ...
    static func managedHookDirectoryFiles(
        ...
        ---
        name: \(hookName)
        description: "Forward OpenClaw internal hook events to AgentIsland"
        metadata:
        ...
        const handler = async (event) => {
          ...
          try {
            ...
            // OpenClaw hooks should never fail because AgentIsland is unavailable.
          }
        };
